...
import { gaussianRandom, clamp, randomBetween } from './utils';
...
export function generateNextReading(machineId: string, injectSpike = false, shadowMode = false): SensorReading {
  ...
  let newTemp = clamp(prev.temperature_C + tempDelta + (drift ? 2 : 0), b.temperature_C.min - 5, b.temperature_C.max + 10);
  let newVib = clamp(prev.vibration_mm_s + vibDelta + (compound ? 0.5 : 0), b.vibration_mm_s.min, b.vibration_mm_s.max + 1.5);
  let newRpm = clamp(prev.rpm + rpmDelta, b.rpm.min - 50, b.rpm.max + 50);
  let newCur = clamp(prev.current_A + curDelta + (compound ? 1 : 0), b.current_A.min, b.current_A.max + 2);
...
export function generateInitialAlerts(): AlertEntry[] {
  ...
  return [
    ...
    },
    {
      ...
      machine_id: 'LATHE_01',
      issue: 'Single-cycle RPM spike — 1920 RPM',
      severity: 'info',
...
export function generateAgentMessages(): { agent: string; message: string; time: string }[] {
  const msgs = [
    ...
    { agent: 'System', message: '--- Stream connected: CNC_01, CNC_02, LATHE_01, PUMP_03 ---' },
